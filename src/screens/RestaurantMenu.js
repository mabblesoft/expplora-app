...
export default function RestaurantMenu() {
			...
			<View style={{ marginBottom: 40, marginTop: 30 }}>
				<FlatList
					...
					renderItem={({ item, index }) => (
						<TouchableOpacity onPress={() => setSelectCategory(index + 1)}>
							<View
								style={{
									width: 70,
									height: 70,
							...
							<Text
								style={{
									...
									color:
										selectCategory == item.id ? COLORS.green : COLORS.gray2,
								}}>
								{item.name}
							</Text>
						</TouchableOpacity>
	...
	return (
		<SafeAreaView style={{ ...SAFEAREAVIEW.AndroidSafeArea }}>
			<Header title='Restaurant menu' onPress={() => navigation.goBack()} />
			<ScrollView
